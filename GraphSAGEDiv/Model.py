...
class InducieveLearningQA(nn.Module):
    def __init__(self, args,
        ...
        if self.args.is_classification:
            ...
            self.w_final = nn.Linear(self.hidden_state_size, 1, bias=False)
    ...
    def forward(self, question, answer_edge, user, need_feature=False):
        ...
        th_user = self.content_cnn(self.word2vec_embed(self.user_embed.content_embed(user)))
        ...
        score = torch.tanh(self.w_a(answer_edge_feaure) + self.w_u(th_user) + self.w_q(th_question))
        if self.args.is_classification:
